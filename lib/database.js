...
exports.checkSecret = function(secret){
	...
	var query = 'select * from secret where secret = ' + secret;
	 ...
	 var rows = pg.connect(process.env.DATABASE_URL, function(err, client, done) {
		var rows = client.query(query, function(err, result) {
			done();
			if (err) {
				console.error(err); console.log('Error ' + err);
			}
			else {
				console.log('Secret found: ' + result.rows);
				return result.rows;
			}
		});
	...
	console.log('Returning secret: ' + rows);
	return rows;
};
