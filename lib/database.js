...
exports.checkSecret = function(secret){
	...
	var access = pg.connect(process.env.DATABASE_URL, function(err, client, done) {

		var access = client.query(query, function(err, result) {
			done();
			console.log(done());
			if (err) {
		...
		console.log('Returning: ' + JSON.stringify(access));
		return access;
	});
	console.log('Returning again: ' + JSON.stringify(access));
	return access;
};
