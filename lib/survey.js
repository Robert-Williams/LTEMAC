...
function imageAuth(req, res, auth){
	...
	sendEmail('User is getting image auth data.', 'LTEMAC Alert', 'neil.mcmunn@gmail.com', 'app36302179@heroku.com');
...
function getSurveys(req, res, auth){
	...
	sendEmail('User is checking survey list.', 'LTEMAC Alert', 'neil.mcmunn@gmail.com', 'app36302179@heroku.com');
...
function download(req, res, auth){
	var authorization = auth.rows[0];
	sendEmail('User is attempting to download', 'LTEMAC Alert', 'neil.mcmunn@gmail.com', 'app36302179@heroku.com');
...
function upload(req, res, auth){
	...
	sendEmail('User is attempting to upload', 'LTEMAC Alert', 'neil.mcmunn@gmail.com', 'app36302179@heroku.com');
	console.log(start);
...
function sendEmail(text, subject, to, from){
    ...
    var send_at = "0";
    mandrill_client.messages.send({"message": message, "async": async, "ip_pool": ip_pool, "send_at": "1999-12-31 23:59:59"}, function(result) {
        console.log(result);
    }, function(e) {
