...
function upload(req, res, auth){
	...
	if(authorization.active && authorization.user_level >= 1){
		...
		if(data.guid && data.survey){
			...
			pg.connect(process.env.DATABASE_URL, function(err, client, done) {
				var query = client.query('select site_survey_guid, version_no, date_surveyed from surveys where site_survey_guid = $1', [data.guid]);
				...
				query.on('end', function(result){
					...
					if(result.rowCount === 0){
						...
						if(auth.rows[0].user_level === 9){
							...
							pg.connect(process.env.DATABASE_URL, function(err, client, done) {
								var query = client.query('update surveys set survey_data = $1, site_survey_guid = $2, date_surveyed = $3, last_modified = $4, version_no = $5 WHERE park_id = $6 AND protocol_id =$7', [JSON.stringify(data.survey), data.guid, data.date, now, versionNumber, data.park, data.protocol]);
