...
function upload(req, res, auth, surveyStatus){
	var authorization = auth.rows[0];
	
	//Check if user is authorized for this action
	if(authorization.active && authorization.user_level > 1){
		var site = req.query.park;
		var protocol = req.query.protocol;
		var survey = req.query.data;	//JSON data to be used for insertion
		...
		// 	survey: req.query.data,
		// 	date: req.query.date,
		// 	version: req.query.version
		// }

		//Check is request parameters are valid
		if(park !== undefined && protocol !== undefined && survey !== undefined){

			//Setup a query to get all surveys from database
			var queryString = 'select site, protocol, version, date_surveyed from surveys where site =\'' + park + '\' and protocol = \'' + protocol + '\'';
			...
			pg.connect(process.env.DATABASE_URL, function(err, client, done, data) {
				...
				query.on('end', function(result){
					...
					date = '';		//Date the survey was created
					modified = '';	//Last date the survey was modified
					...
					if(result.rowCount === 0){
						...
						if(auth.user_level === 9 || ){
							var queryString = 'update surveys set site = ' + data.site + ', protocol = '  + data.protocol + ', survey_data = \'' + data.survey + '\', date_surveyed =' + data.date + ', last_modified =' + data.modified + ', version_no =' + (data.version++) ')';
							pg.connect(process.env.DATABASE_URL, function(err, client, done) {
								var query = client.query(queryString);
